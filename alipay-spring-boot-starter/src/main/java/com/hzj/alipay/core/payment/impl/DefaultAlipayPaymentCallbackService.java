...
public class DefaultAlipayPaymentCallbackService implements AlipayPaymentCallbackService {
    ...
    private final AlipayPaymentStaticConfigProvider provider;
    ...
    public AliPayCallbackResult parseCallback(HttpServletRequest request, HttpServletResponse response) {
        try {
            AlipayPaymentStaticConfig config = getAlipayConfig();
     ...
     * @return 验签后的回调结果
     */
    private AliPayCallbackResult verifyCallback(HttpServletRequest request, AlipayPaymentStaticConfig config) {
        if (request == null) {
     ...
     * @param config 支付宝配置
     */
    private void validateCallback(AliPayCallbackResult callbackResult, AlipayPaymentStaticConfig config) {
     ...
     * @return 支付宝配置
     */
    private AlipayPaymentStaticConfig getAlipayConfig() {
        if (provider == null) {
            throw new IllegalStateException("AlipayPaymentStaticConfigProvider 未初始化");
        }
        AlipayPaymentStaticConfig config = provider.getConfig();
        if (config == null) {
