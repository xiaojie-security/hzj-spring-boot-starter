 ...
 * 微信分账自动装配配置。
 * <p>
 * 仅当使用方提供了 {@link WechatPaymentStaticConfigProvider} Bean 时才会装配。
 */
@AutoConfiguration
@ConditionalOnBean(WechatPaymentStaticConfigProvider.class)
public class WechatProfitsharingConfiguration {


    @Bean
    @ConditionalOnMissingBean(WechatProfitsharingService.class)
    public WechatProfitsharingService wechatProfitsharingService(WechatPaymentStaticConfigProvider provider){
